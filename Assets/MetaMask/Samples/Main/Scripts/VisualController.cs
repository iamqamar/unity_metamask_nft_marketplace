...
namespace MetaMask.Unity.Samples
{
    ...
    public class VisualController : MonoBehaviour
    {
        ...
        public bool enableResultModal = true;
        private bool scale;
        ...
        {
            MainThreadUtil.Instance.StartCoroutine(WalletReadyUI());
        }

        private IEnumerator WalletReadyUI()
        {
            // Wait for new frame
        ...
        private void OnWalletDisconnected(object sender, EventArgs e)
        {
            ...
            // Show reconnect screen on desktop
            if (wallet.Transport.ConnectionMode != TransportMode.Deeplink || !wallet.HasSession)
            {
                WalletStopVisuals();
            }
            
            if (wallet.Transport.ConnectionMode == TransportMode.Deeplink)
            {
